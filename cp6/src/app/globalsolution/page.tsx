...
export default function Globals() {
    ...
    const handleDelete = async (id: number) => {
        try {
            ...
            if (response.ok) {
                alert("A Global Solution foi excluída com sucesso!");
    ...
    return (
        <div className="min-h-screen bg-blue-950 p-8">
            <h1 className="text-4xl font-bold text-center text-white mb-6">Global Solution</h1>
            <p className="text-center text-lg text-gray-300 mb-8">Global Solutions são projetos de duração de duas semanas que concede 60% da nota</p>
                    ...
                    <thead className="bg-blue-700 text-white">
                        <tr>
                            ...
                            <th className="py-3 px-6 text-left">
                                <Link href={"/globalsolution/cad-globalsolution"} className="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600">CADASTRAR</Link>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        {globalsFiltrados.map((p) => (
                            <tr key={p.id} className="border-b hover:bg-gray-100">
                                ...
                                <td className="py-3 px-6">
                                    <Link href={`/globalsolution/globalsolution/${p.id}`} className="text-blue-500 hover:underline mr-2">EDITAR</Link> | 
                                    <a
                                        href="#"
